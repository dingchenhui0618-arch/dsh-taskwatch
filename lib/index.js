...
// 离线策略有意做得「保守」：外壳可离线打开，但状态数据永远走网络。
// 对监控而言，看到过期状态比看到断连更危险。
const SERVICE_WORKER = `const SHELL=['/taskwatch','/taskwatch/icon-192.png','/taskwatch/icon-512.png']
const CACHE='taskwatch-v1'
...
self.addEventListener('fetch',function(e){
 ...
 if(u.pathname==='/taskwatch/data'){
  e.respondWith(fetch(e.request).catch(function(){
   return new Response(JSON.stringify({generatedAt:Date.now(),offline:true,errors:['离线：读不到 DSH，数据未刷新'],
...
export function apply(ctx) {
  ...
  if (webServer !== undefined) {
    ...
    console.error('dsh-taskwatch: 没有 webServer 服务，两个 HTTP 路由未注册')
  }
}
